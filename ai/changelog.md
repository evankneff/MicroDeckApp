...
## Extra — App Icon
*Feb 23, 2026 · Not tied to a roadmap phase*
...
## Phase 5/6 — Voice Input FAB Wiring
*Mar 30, 2026 · Related plans: [ai/roadmaps/2026-03-28-phase-5-daily-refresh.md](roadmaps/2026-03-28-phase-5-daily-refresh.md), [ai/roadmaps/2026-03-28-phase-6-tiny-start-unified.md](roadmaps/2026-03-28-phase-6-tiny-start-unified.md)*

- `lib/screens/deck/deck_screen.dart` — added `_AddMethod` enum (`voice`, `manual`) for sheet result signaling
- `lib/screens/deck/deck_screen.dart` — added `_AddMethodSheet` private widget: bottom sheet with "Use voice" (filled) and "Type it" (outlined) options, consistent with existing sheet styling
- `lib/screens/deck/deck_screen.dart` — added `_showAddMethodSheet()` method: shows `_AddMethodSheet`, routes `voice` result to existing `_openVoiceInput()` and `manual` result to `CreateCardGoalScreen()`
- `lib/screens/deck/deck_screen.dart` — FAB `onPressed` changed from direct `CreateCardGoalScreen` navigation to `_showAddMethodSheet`
- `lib/screens/deck/deck_screen.dart` — empty-state "Add a card" button `onPressed` changed to `_showAddMethodSheet` for consistency
- Pre-existing `_openVoiceInput()`, `VoiceInputSheet`, and Fresh Start "Voice planning" button are unchanged
- `flutter analyze` — no issues
